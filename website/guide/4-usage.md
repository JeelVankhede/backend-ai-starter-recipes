...
```text
api/
├── .cursor/rules/     # AI rules — Cursor (if selected)
├── .cursor/skills/    # Lifecycle stages — Cursor
├── CLAUDE.md          # AI rules pointer index — Claude Code (if selected)
├── .claude/rules/     # AI rules — Claude Code
└── ...
```

No `.ai/` directory is created. (Preset defaults include specific IDEs; customize via interactive run or by editing a copied preset JSON for your fork.)
