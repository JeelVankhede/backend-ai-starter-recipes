...
const CONFIG_PATH = path.join(os.homedir(), '.config', 'bare', 'config.json');
...
export async function resolveTrackingConsent(): Promise<boolean> {
  ...
  console.log(`
${chalk.bold.magenta('📊  Anonymous usage tracking')}

  ${chalk.dim('bare collects anonymous data to understand how the tool is used')}
  ${chalk.dim('and where to focus improvements. No personal data is ever collected.')}

  ${chalk.bold('What is tracked:')}
    ${chalk.green('·')} Preset name              e.g. nestjs-prisma
    ...
    ${chalk.green('·')} Node.js version, OS platform, bare version
  ...
  ${chalk.dim('~/.config/bare/config.json')}
`);

  const allowed = await confirm({
    message: 'Allow anonymous usage tracking to help improve bare?',
    default: true,
  });
  ...
  console.log(
    allowed
      ? chalk.dim('\n  Thanks. You can opt out any time by editing ~/.config/bare/config.json\n')
      : chalk.dim('\n  No data will be collected. You can change this any time via ~/.config/bare/config.json\n'),
  );
