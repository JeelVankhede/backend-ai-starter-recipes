...
describe('BANNER_TITLE', () => {
  it('uses the binding Bare casing (lowercase i + starter, capital RE)', () => {
    expect(BANNER_TITLE).toBe('Bare — Backend Ai starter REcipes');
  });
});
...
describe('renderStartupBanner', () => {
  it('includes title, version, description, and docs URL as plain text outside the box', () => {
    ...
    const boxLines = lines.slice(0, -1);
